...
const Actions = ({ id }: IActions): JSX.Element => {
  ...
  const {
    ...
    removeDisabledProductId,
  } = useInventoryManagementStore((state) => ({
    view: state.view,
  ...
  const onActionClick = (action: Action) => {
    switch (action) {
      case Action.Edit:
        ...
        break;
      case Action.View:
        if (disabledProductIds.includes(id)) removeDisabledProductId(id);
  ...
  const getStyle = (color: string) => ({ color, cursor: "pointer" });
  ...
  const getSx = (action: Action) => {
    ...
    switch (action) {
      case Action.Edit:
        ...
        return getStyle("#377c22");
      case Action.View:
        return getStyle("#b98ec6");
      case Action.Delete:
        return getStyle("#eb3323");
    }
  };
  ...
  return (
    <>
      <div className="actions">
        <EditIcon
          sx={getSx(Action.Edit)}
          onClick={() => onActionClick(Action.Edit)}
        />
        {disabledProductIds.includes(id) ? (
          <VisibilityOffIcon
            sx={getSx(Action.View)}
            onClick={() => onActionClick(Action.View)}
          />
        ) : (
          <VisibilityIcon
            sx={getSx(Action.View)}
        ...
        <DeleteIcon
          sx={getSx(Action.Delete)}
          onClick={() => onActionClick(Action.Delete)}
        />
      </div>
      {showEditModal ? (
        <EditModal show={showEditModal} setShow={setShowEditModal} id={id} />
      ) : null}
    </>
  );
};
